...
public class OrderStatusTransaction {
    ...
    public void execute() {
        try {
            System.out.println(
                    String.format("------Order Status: warehouse id: %d, district id: %d, customer id: %d------",
                            this.warehouseID, this.districtID, this.customerID));

            int orderID = -1;

            System.out.println("---Customer Details: ");
            ResultSet customer = this.session.execute(
                    String.format(QUERY_GET_CUSTOMER_DETAILS, this.warehouseID, this.districtID, this.customerID));
            ...
            ResultSet lastOrder = this.session
                    .execute(String.format(QUERY_GET_LAST_ORDER, this.warehouseID, this.districtID, this.customerID));
            ...
            ResultSet itemDetails = this.session
                    .execute(String.format(QUERY_GET_ITEM_DETAILS, this.warehouseID, this.districtID, orderID));
            for (Row row : itemDetails.all()) {
                ...
                System.out.printf(
                        "Item Number: %d, Supplying Warehouse Number: %d, Quantity: %d, Amount: %d, Delivery Time: %s\n",
